...
export function logKpi(event, detail = {}) {
  // Telemetry must never prevent a recognized sign from reaching the conversation.
  try {
    const entries = JSON.parse(localStorage.getItem(KEY) || "[]");
    entries.push({ event, timestamp: new Date().toISOString(), ...detail });
    localStorage.setItem(KEY, JSON.stringify(entries.slice(-250)));
  } catch { /* Storage may be unavailable in private browsing or full. */ }
}
