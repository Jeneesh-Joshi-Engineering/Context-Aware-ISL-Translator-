...
public class ConversationWebSocketHandler extends TextWebSocketHandler {
    ...
    private final SessionManager sessionManager; private final GeminiClient geminiClient; private final KpiLogger kpiLogger; private final ObjectMapper mapper;
    public ConversationWebSocketHandler(SessionManager sessionManager, GeminiClient geminiClient, KpiLogger kpiLogger, ObjectMapper mapper) { this.sessionManager = sessionManager; this.geminiClient = geminiClient; this.kpiLogger = kpiLogger; this.mapper = mapper; }
    ...
    private void processKeywords(WebSocketSession sender, String sessionId, IncomingMessage input) {
        ...
        geminiClient.generate(keywords).whenComplete((sentence, failure) -> {
    ...
    private List<String> sanitize(IncomingMessage input) {
        ...
        for (String value : values) { if (value == null) continue; String word = value.trim().replaceAll("[^\\p{L}\\p{N} .,'!?-]", "").replaceAll("\\s+", " "); if (!word.isBlank() && word.length() <= 80) clean.add(word); }
